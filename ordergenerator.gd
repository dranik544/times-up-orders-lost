...
func _pick_template() -> Dictionary:
	...
	for order in OrderList.orders:
		...
		if otype == null:
			...
			candidates.append(order)
		elif otype == 3 and completed >= 35:
			candidates.append(order)
		elif otype == 4 and completed >= 20:
			candidates.append(order)
		elif otype == 5:
			candidates.append(order)
		else:
	...
	# Если по тегу ничего не нашлось — берём заказы без типа (DEFAULT)
	if candidates.empty():
		for order in OrderList.orders:
			if order.get("type", null) == null:
				candidates.append(order)
	
	# Если всё равно пусто — берём вообще любые заказы (последний шанс)
	if candidates.empty():
		candidates = OrderList.orders.duplicate()
...
func _process_template(template: Dictionary) -> Dictionary:
	...
	var max_groups = groups.size()
	if max_groups > 4:
		max_groups = 4
	var count = randi() % max_groups + 1
	var selected = groups.duplicate()
	selected.shuffle()
	selected = selected.slice(0, count)  # было slice(0, count-1)

	var flat_prms = []
	var group_texts = []

	for group in selected:
		# Генерируем значения для frmt
		var generated = {}
		if group.has("frmt"):
			for key in group["frmt"]:
				var value = _generate_value(group["frmt"][key])
				# Если это словарь с ключами text и index, раскладываем
		...
		# Формируем текст группы с подстановкой значений
		var group_desc = group.get("desc", "")
		for key in generated:
			group_desc = group_desc.replace("{" + key + "}", str(generated[key]))
		...
		for block in group.get("blck", []):
			...
			for field in b.keys():
				if typeof(b[field]) == TYPE_STRING:
					for key in generated:
						b[field] = b[field].replace("{" + key + "}", str(generated[key]))
